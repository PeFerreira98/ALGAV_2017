...
%br_fir( Start, Goal, Path):- bfs([[Start]], Goal, Path).

%bfs([[Visited|Rest]|_], Goal, Path) :- reverse ([Visited|Rest], Path).

% bfs([Path|Rest], Goal, Sol) :- estend(Path, NewPath), append(Rest,
% NewPath, Path), bfs(Path, Goal, Sol).

%estend([Node|Path], NewPath) :- findall([NewNode, Node |Path],(fronteira(Node, NewNode);fronteira(NewNode, Node)) not(member(NewNode, Path), NewPath)).
